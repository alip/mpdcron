...
"http://www.sqlite.org/lang_expr.html");
	...
	if (expr != NULL) {
		if ((db = db_init(euconfig.dbpath)) ==  NULL)
			return 1;
		if (opta)
			ret = db_kill_artist_expr(db, expr, true,
					(euconfig.verbosity > LOG_WARNING));
		else if (optA)
			ret = db_kill_album_expr(db, expr, true,
					(euconfig.verbosity > LOG_WARNING));
		else if (optg)
			ret = db_kill_genre_expr(db, expr, true,
					(euconfig.verbosity > LOG_WARNING));
		else
			ret = db_kill_song_expr(db, expr, true,
					(euconfig.verbosity > LOG_WARNING));
		sqlite3_close(db);
		return ret ? 0 : 1;
	}
	...
	if (opta)
		ret = db_kill_artist(db, song, true);
	else if (optA)
		ret = db_kill_album(db, song, true);
	else if (optg)
		ret = db_kill_genre(db, song, true);
	else
		ret = db_kill_song(db, song, true);
	sqlite3_close(db);
	mpd_song_free(song);
	return ret ? 0 : 1;
}
