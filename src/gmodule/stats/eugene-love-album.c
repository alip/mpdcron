...
static GOptionEntry options[] = {
	...
	{"expr", 'e', 0, G_OPTION_ARG_STRING, &expr, "Love albums matching the given expression", NULL},
...
static int love_current(void)
{
	...
	if ((song = load_current_song()) == NULL)
		return 1;
	ret = db_love_album(euconfig.dbpath, song, true);
	mpd_song_free(song);
	return ret ? 0 : 1;
}
...
int cmd_love_album(int argc, char **argv)
{
	...
	g_option_context_add_main_entries(ctx, options, "eugene-love-album");
	g_option_context_set_summary(ctx, "eugene-love-album-"VERSION GITHEAD" - Love album");
...
"http://www.sqlite.org/lang_expr.html");
	...
	if (expr != NULL)
		return db_love_album_expr(euconfig.dbpath, expr, true) ? 0 : 1;
	else
		return love_current();
}
