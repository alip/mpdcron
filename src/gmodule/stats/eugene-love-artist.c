...
static GOptionEntry options[] = {
	...
	{"expr", 'e', 0, G_OPTION_ARG_STRING, &expr, "Love artists matching the given expression", NULL},
...
static int love_current(void)
{
	...
	if ((song = load_current_song()) == NULL)
		return 1;
	ret = db_love_artist(euconfig.dbpath, song, true);
	mpd_song_free(song);
	return ret ? 0 : 1;
}
...
int cmd_love_artist(int argc, char **argv)
{
	...
	g_option_context_add_main_entries(ctx, options, "eugene-love-artist");
	g_option_context_set_summary(ctx, "eugene-love-artist-"VERSION GITHEAD" - Love artist");
...
"http://www.sqlite.org/lang_expr.html");
	...
	if (expr != NULL)
		return db_love_artist_expr(euconfig.dbpath, expr, true) ? 0 : 1;
	else
		return love_current();
}
