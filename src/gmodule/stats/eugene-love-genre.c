...
static GOptionEntry options[] = {
	...
	{"expr", 'e', 0, G_OPTION_ARG_STRING, &expr, "Love genres matching the given expression", NULL},
...
static int love_current(void)
{
	...
	if ((song = load_current_song()) == NULL)
		return 1;
	ret = db_love_genre(euconfig.dbpath, song, true);
	mpd_song_free(song);
	return ret ? 0 : 1;
}
...
int cmd_love_genre(int argc, char **argv)
{
	...
	g_option_context_add_main_entries(ctx, options, "eugene-love-genre");
	g_option_context_set_summary(ctx, "eugene-love-genre-"VERSION GITHEAD" - Love genre");
