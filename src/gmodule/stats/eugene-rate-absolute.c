...
static int
rate_absolute_artist(struct mpdcron_connection *conn, const char *expr,
		const char *rating)
{
	int changes;
	...
	if (expr != NULL) {
		if (!mpdcron_rate_absolute_artist_expr(conn, expr, rating, &changes)) {
		...
		if (!mpdcron_rate_absolute_artist_expr(conn, myexpr, rating, &changes)) {
...
static int
rate_absolute_album(struct mpdcron_connection *conn, const char *expr,
		const char *rating)
{
	int changes;
	...
	if (expr != NULL) {
		if (!mpdcron_rate_absolute_album_expr(conn, expr, rating, &changes)) {
		...
		if (!mpdcron_rate_absolute_album_expr(conn, myexpr, rating, &changes)) {
...
static int
rate_absolute_genre(struct mpdcron_connection *conn, const char *expr,
		const char *rating)
{
	int changes;
	...
	if (expr != NULL) {
		if (!mpdcron_rate_absolute_genre_expr(conn, expr, rating, &changes)) {
		...
		if (!mpdcron_rate_absolute_genre_expr(conn, myexpr, rating, &changes)) {
...
static int
rate_absolute_song(struct mpdcron_connection *conn,
		const char *expr, const char *rating)
{
	...
	if (expr != NULL) {
		if (!mpdcron_rate_absolute_expr(conn, expr, rating, &changes)) {
		...
		if (!mpdcron_rate_absolute_expr(conn, myexpr, rating, &changes)) {
...
static int
cmd_rate_absolute_internal(const char *expr, const char *rating,
		bool artist, bool album, bool genre)
{
	...
	if (artist)
		ret = rate_absolute_artist(conn, expr, rating);
	else if (album)
		ret = rate_absolute_album(conn, expr, rating);
	else if (genre)
		ret = rate_absolute_genre(conn, expr, rating);
	else
		ret = rate_absolute_song(conn, expr, rating);
	mpdcron_connection_free(conn);
	return ret;
}
...
cmd_rate_absolute(int argc, char **argv)
{
	...
	g_option_context_add_main_entries(ctx, options, "eugene-rate-absolute");
	g_option_context_set_summary(ctx, "eugene-rate-absolute"VERSION GITHEAD" - Rate (absolute fashion) song/artist/album/genre");
...
"http://www.sqlite.org/lang_expr.html");
	...
	if (argc <= 1) {
		...
		ret = cmd_rate_absolute_internal(argv[2], argv[1], opta, optA, optg);
	else
		ret = cmd_rate_absolute_internal(NULL, argv[1], opta, optA, optg);
	return ret;
}
