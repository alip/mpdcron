...
static int
cmd_rmtag_internal(const char *tag, const char *expr,
		bool artist, bool album, bool genre)
{
	...
	if (artist)
		ret = rmtag_artist(conn, tag, expr);
	else if (album)
		ret = rmtag_album(conn, tag, expr);
	else if (genre)
		ret = rmtag_genre(conn, tag, expr);
	else
		ret = rmtag_song(conn, tag, expr);
	mpdcron_connection_free(conn);
	return ret;
}
