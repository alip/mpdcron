...
static int run_update(const char *path)
{
	...
	while ((entity = mpd_recv_entity(conn)) != NULL) {
		if (mpd_entity_get_type(entity) == MPD_ENTITY_TYPE_SONG) {
			...
			if (!db_process(euconfig.dbpath, song, false))
				return -1;
			++nsongs;
		}
...
int applet_update(int argc, char **argv)
{
	int ret;
	GOptionContext *ctx;
	...
	g_option_context_set_summary(ctx, "eugene-update-"VERSION GITHEAD" - update applet");
	...
	if (ret > 0) {
		eulog(LOG_NOTICE, "Successfully processed %d songs",
				ret, euconfig.dbpath);
		return 0;
	}
	return -1;
