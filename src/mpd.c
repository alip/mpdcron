...
gint mhmpd_connect(void)
{
    for (unsigned int try = 1 ; ; try++) {
        daemon_log(LOG_INFO, "Connecting to `%s' on port %s... (try: %d)", mhconf.hostname, mhconf.port, try);
        mhconf.conn = mpd_connection_new(mhconf.hostname, atoi(mhconf.port), 10);
        if (!mhconf.conn) {
            if (mhconf.reconnect > 0) {
                daemon_log(LOG_ERR, "Failed to connect, retrying in %d seconds!", mhconf.reconnect);
            ...
            if (mhconf.reconnect > 0) {
                daemon_log(LOG_ERR, "Failed to connect: %s, retrying in %d seconds!",
        ...
        daemon_log(LOG_INFO, "Connected to `%s' on port %s.", mhconf.hostname, mhconf.port);
